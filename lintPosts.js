...
const logError = (...params) => console.error(errorColour(...params));

const logWarning = (...params) => console.warn(warningColour(...params));
    ...
    paths.forEach((path) => {
      try {
        ...
        if (frontMatter.data.category) {
          ...
          frontMatterCats &&
          frontMatterCats.length &&
          frontMatterCats.length <= MAX_CATEGORIES
        ) {
          postCategories = frontMatter.data.categories.map((c) =>
            c.toLowerCase(),
          );
          ...
          logError(
            "The post " +
              path +
              " does not have at least one and no more than " +
              MAX_CATEGORIES +
              " categories defined.",
          );
          fail = true;
        ...
        if (!categories.includes(category)) {
          logError("The post " + path + " does not have a recognised category");
          fail = true;
        } else {
          postCategories
            ...
            .forEach((c) =>
              logWarning(
                "The post " +
                  ...
                  c +
                  "'. Check spelling or remove/move to tags.",
              ),
            );
        }
          ...
          if (!summary) {
            ...
            logWarning(
              "The post " +
                ...
                " summary length is " +
                summary.length +
                ". Recommended minimum length for the summary is 130 characters.",
            );
          }
        }
      } catch (e) {
