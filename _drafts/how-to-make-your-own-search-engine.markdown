...
The embedding vector has many values (~3000 values for the [CLS] embedding) each of which represent the strength of some category in a range from 0 to 1. These categories [don't always have a clear meaning ](http://www.cs.cmu.edu/~bmurphy/NNSE/) because each value represents a category that the LLM decided in training. However, when you represent the vector in space, words or sentences with similar meaning are clustered together. To understand the values of an embedding, we would need to use a method called [feature extraction](https://towardsdatascience.com/feature-extraction-with-bert-for-text-classification-533dde44dc2f) using techniques like [PCA or tSNE](https://www.thekerneltrip.com/statistics/tsne-vs-pca/) to reduce these large embedding vectors to more simple plots. 
