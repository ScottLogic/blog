...
On the client project, there were at least three different contexts that applied the Transactional Outbox pattern. In the first context, the data model needed to be relational. Data consistency was required between a component that had REST endpoints and an external system. This involved using PostgreSQL as the datastore and implementing a polling layer to the component, where the was a guarantee that only a single instance would be polling. The code for the REST endpoints was simple, just writing the data and message payload in the same transaction and returning a status code. The team implementing this were able to keep the complexity low in the polling layer, by polling for new message payloads from the "outbox", removing messages when processed and marking any messages as failed if the external system encountered an issue.

The second context, like the first needed a relational data model, however the data consistency was going across an event pipeline. This also used PostgreSQL, yet due to the forecasted performance measures, it was not considered viable to have a single instance. The team implementing this were able to use the row-level locking capabilities of PostgreSQL to achieve their needs. This row-level locking did require more development and testing effort.

The third context, which I worked in, was event pipeline where our data model requirements were document-oriented. We used Azure Cosmos as the datastore and could its change feed capability to abstract away the tracking of changes. Additionally, each document can be given a time-to-live (TTL) attribute so that old records could be automatically cleaned up. There was some complexity added to reading from containers in Azure Cosmos, as we had to place data and event payloads in the same container to get transactional guarantees.
