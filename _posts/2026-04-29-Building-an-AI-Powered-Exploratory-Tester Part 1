...
My first attempt was revealing. I built the testing tool in the same repository as the target application (one of my earliest experiments with AI dev tools). Copilot, with full access to the app's source code, followed its natural instinct: it generated a set of role-based Playwright scripts and wired them into a CI pipeline. Solid, deterministic, conventional test automation. Exactly what you'd expect from a tool that can see every route, every component, every permission check in the codebase.
