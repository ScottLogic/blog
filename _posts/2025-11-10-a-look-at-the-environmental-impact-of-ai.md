...
![A smartphone on top of a book on problem-solving agents, the phone displays OpenAI's ChatGPT. ]({{ site.baseurl }}/hsauve/assets/content/chatgpt.jpg "A smartphone displaying OpenAI's ChatGPT.")
...
AI is increasing demand in AI chips and analysts have estimated that demand for Nvidia's prized AI chips is exceeding supply by at least 50%. [^5]

The embodied carbon of AI is far from negligeable. A comprehensive AI cradle-to-grave approach [^11] estimates that manufacturing emissions represent under 25% of AI carbon emissions and data centre construction emissions under 5%. Such an approach is necessary to not lose sight of natural resources depletion, pollution and biodiversity loss associated with the development of AI systems. [^8]
...
Inference does not only apply to chatbots, but is also increasingly integrated into systems such as search engines. As an example, Alphabet’s chairman indicated in February 2023 that interacting with an LLM could "likely cost 10 times more than a standard keyword search". [^4]
...
To provide a sense of scale, training GPT-3 is estimated to have consumed 1,287 megawatt-hours (MWh) of electricity and emitted over 550 metric tons of CO2e [^2], and evaporated 700,000 litres of clean freshwater [^3], enough to fill an Olympic-sized swimming pool by nearly one-third.
...
Researchers noticed that on average the A100 GPU decreased model training time by 62.6%. [^12]
...
- Frugal AI innovation meaning resource-conscious systems that are functional, robust, user-friendly, growing, affordable, and local. [^8].
- Federated Learning offers a solution by decentralising the
training process and offers several advantages such as reducing the time and bandwidth required for training and inference and lower the energy consumption associated with long-distance data transmission. [^7]
- [efficiency should be an evaluation criterion so that ML practitioners compete to increase accuracy](https://arxiv.org/abs/2104.10350). Although rebound effect: https://www.cam.ac.uk/stories/green-algorithms

3) Use the right AI for the right need at the right time [^8]
> Artificial intelligence should only be used in cases where it is the best technique to use.
- Artificial intelligence should only be used in cases where it is the best technique to use [^8]. Challenge the necessity
...
- Use the right AI for the right need at the right time [^8]
- Reduce training time - also helps more people to take part. [NLP researchers](https://www.techcarbonstandard.org/resources/glossary#natural-language-processing-nlp) don't have access to the same [computational powers](https://arxiv.org/abs/1906.02243). They should prioritise developing efficient models and hardware. Improvements in state of the art accuracy are possible thanks to industry access to large-scale compute.
- Choose carefully the DNN model, processor and data centre
- Lighten existing models and use faster GPUs [^12] ass a necessary step to both reduce the environmental damage of LLM training while maintaining results. Although this comes with financial implications, which necessitates further research to make sustainable AI practices more accessible.
...
- Long LLM reasoning does not mean accuracy: The study Don't Overthink it examines the relationship between reasoning trajectory length and correctness in reasoning LLMs and challenge the assumption of a correlation between increased average thinking length during inference and improved model performance. In their experimentations, the researchers observed that correct answers are typically shorter than incorrect ones. Training models on shorter reasoning sequences and advocate for models that reason more effectively with less computation. [^10]
- opt for lighter models (BERT)


### what should be taken into account?

AI’s energy footprint depends
on factors like data center location, energy mix, model
complexity, and training duration
- ML model: algorithm. improve efficiency using techniques
- programme that implements the model
- number of processors that run the programme
- speed and powers of those processors
- data centre's efficieny in delivering power and cooling the processors
- energy supply mix
...
[^2]: Jegham, N., Abdelatti, M., Elmoubarki, L., & Hendawi, A. (2025). “How Hungry is AI? Benchmarking Energy, Water, and Carbon Footprint of LLM Inference”. University of Rhode Island, University of Tunis, Providence College.

[^3]: Pengfei Li, Jianyi Yang, Mohammad A. Islam, Shaolei Ren (2025). "Making AI Less “Thirsty: Uncovering and Addressing the Secret Water Footprint of AI Models". UC Riverside, UT Arlington. [https://doi.org/10.48550/arXiv.2304.03271](https://doi.org/10.48550/arXiv.2304.03271)

[^4]: Dastin, J., and Nellis, S. (2023). Focus: "For tech giants, AI like Bing and Bard poses billion-dollar search problem". Reuters. February 22, 2023. https://www.reuters.com/technology/tech-giants-ai-like-bing-bard-poses-billion-dollar-search-problem-2023-02-22/.

[^5]: Chavi Mehta, Max A. Cherney and Stephen Nellis "Nvidia adds jet fuel to AI optimism with record results, $25 billion buyback". Reuters. August 24, 2023. https://www.reuters.com/technology/nvidia-forecasts-third-quarter-revenue-above-wall-street-expectations-2023-08-23/

[^6]: Xiaorong Wang, Clara Na, Emma Strubell, Sorelle Friedler, Sasha Luccioni (2023). "Energy and Carbon Considerations of Fine-Tuning BERT"[https://doi.org/10.48550/arXiv.2311.10267](https://doi.org/10.48550/arXiv.2311.10267)

[^7]: Iftikhar, S., Alsamhi, S. H., & Davy, S. (2025). “Enhancing Sustainability in LLM Training: Leveraging Federated Learning and Parameter-Efficient Fine-Tuning”. “IEEE Transactions on Sustainable Computing. [https://doi.org/10.1109/TSUSC.2025.3592043](https://doi.org/10.1109/TSUSC.2025.3592043).

[^8]: Ludovic Arga, François Bélorgey, Arnaud Braud, Romain Carbou, Nathalie Charbonniaud, et al. Frugal AI: Introduction, Concepts, Development and Open Questions. 2025. ffhal-05049765f

[^9]: Kai Huang, Hanyun Yin, Heng Huang‡ & Wei Gao (2024). "Towards Green AI in Fine-tuning Large Language Models via Adaptive Backpropagation". [https://doi.org/10.48550/arXiv.2309.13192](https://doi.org/10.48550/arXiv.2309.13192)

[^10]: Michael Hassid, Gabriel Synnaeve, Yossi Adi, Roy Schwartz (2025). "Don't Overthink it. Preferring Shorter Thinking Chains for Improved LLM Reasoning". [https://doi.org/10.48550/arXiv.2505.17813](https://doi.org/10.48550/arXiv.2505.17813)

[^11]: Ian Schneider, Hui Xu, Stephan Benecke, David Patterson, Keguo Huang, Parthasarathy Ranganathan, Cooper Elsworth (2025) "Life-Cycle Emissions of AI Hardware: A Cradle-To-Grave Approach and Generational Trends" [https://doi.org/10.48550/arXiv.2502.01671](https://doi.org/10.48550/arXiv.2502.01671)

[^12]: Vivian Liu, Yiqiao Yin (2024). "Green AI: Exploring Carbon Footprints, Mitigation Strategies, and Trade Offs in Large Language Model Training" [https://doi.org/10.48550/arXiv.2404.01157](https://doi.org/10.48550/arXiv.2404.01157)