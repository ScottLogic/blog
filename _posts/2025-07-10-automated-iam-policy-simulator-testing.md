...
There is another simulation method, `simulate custom policy`. This, too, won't work with resource policies if using a role entity, but as we provide the policies for the simulation rather than the simulator fetching them from actual entities itself, we can tell it we're testing a user ARN while supplying our role's policy and it won't notice.
